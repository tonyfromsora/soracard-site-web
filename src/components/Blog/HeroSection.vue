...
<template>
  <section class="hero relative text-center pt-3xl pb-xl">
    <div class="w">
      <h1 class="mb-s">
        {{ title }}
      </h1>
      <p class="text-l mb-m mx-auto">
        {{ description }}
      </p>
      <p class="date text-s color-dark2 mb-l mx-auto">
      ...
      <img :src="image" alt="Article cover image" class="rounded">
    </div>
  </section>
</template>
...
.hero::before {
  ...
  top: 0;
  right: 0;
  bottom: calc(min(100vw, 128rem) * 0.26 + var(--space-xl));
  left: 0;
  z-index: -1;
}
